...
import Review from '../../libs/components/pet/Review';
...
import { GET_COMMENTS, GET_PETS, GET_PET } from '../../apollo/user/query';
...
const PetDetail: NextPage = ({ initialComment, ...props }: any) => {
	...
	const [destinationPets, setDestinationPets] = useState<Pet[]>([]);
	const [commentInquiry, setCommentInquiry] = useState<CommentsInquiry>(initialComment);
	const [petComments, setPetComments] = useState<Comment[]>([]);
	...
	[
		...
		{ featureType: 'water', stylers: [{ color: '#b2ebf2' }] },
		{ featureType: 'road', stylers: [{ visibility: 'off' }] },
		{ featureType: 'poi.park', stylers: [{ color: '#a5d6a7' }] },
	];
	...
	const {
		...
		variables: { input: petId },
		skip: !petId,
		notifyOnNetworkStatusChange: true,
		onCompleted: (data: T) => {
			if (data?.getPet) setPet(data.getPet);
			if (data.getPet) setSlideImage(data.getPet?.petImages[0]);
		},
	});
	...
	const {
		...
		variables: {
			input: {
				...
				search: {
					locationList: pet?.petLocation ? [pet?.petLocation] : [],
				},
			},
		},
		skip: !petId && !pet,
		notifyOnNetworkStatusChange: true,
		onCompleted: (data: T) => {
			if (data?.getPets?.list) setDestinationPets(data?.getPets?.list);
		},
	});
	...
	const {
		...
		notifyOnNetworkStatusChange: true,
		onCompleted: (data: T) => {
			if (data?.getComments?.list) setPetComments(data?.getComments?.list);
	...
	useEffect(() => {
		if (router.query.id) {
			setPetId(router.query.id as string);
			setCommentInquiry({
				...commentInquiry,
				search: {
	...
	if (device === 'mobile') {
		...
		return (
			<div id={'pet-detail-page'}>
				<div className={'container'}>
					<Stack className={'pet-detail-config'}>
						<Stack className={'pet-info-config'}>
							<Stack className={'info'}>
								<Stack className={'left-box'}>
									<Typography className={'title-main'}>{pet?.petTitle}</Typography>
									<Stack className={'top-box'}>
										...
										<Typography className={'date'}>{moment().diff(pet?.createdAt, 'days')} days ago</Typography>
									</Stack>
									...
									<Stack className="buttons">
										...
										<Stack className="button-box">
											<IconButton
												color={'default'}
												// @ts-ignore
												onClick={() => likePetHandler(user, pet?._id)}
											>
											...
											<Typography>{pet?.petLikes}</Typography>
										</Stack>
									</Stack>
									<Typography sx={{ background: '#7ed957', borderRadius: '12px' }}>
							...
							<Stack className={'images'}>
								<Stack className={'main-image'}>
									<img
										src={slideImage ? `${REACT_APP_API_URL}/${slideImage}` : '/img/pet/bigImage.png'}
										alt={'main-image'}
									/>
								</Stack>
								<Stack className={'sub-images'}>
									{pet?.petImages.map((subImg: string) => {
						...
						<Stack className={'pet-desc-config'}>
							<Stack className={'right-config'}>
										...
										<img
											...
											src={
												pet?.memberData?.memberImage
													? `${REACT_APP_API_URL}/${pet?.memberData?.memberImage}`
													: '/img/profile/defaultUser.svg'
											}
										/>
										<Stack className={'name-phone-listings'}>
											<Link href={`/member?memberId=${pet?.memberData?._id}`}>
												<Typography className={'name'}>{pet?.memberData?.memberNick}</Typography>
											</Link>
											<Stack className={'phone-number'}>
												...
												<Typography className={'number'}>{pet?.memberData?.memberPhone}</Typography>
											</Stack>
								...
								<Stack className={'info-box'}>
									...
									<textarea placeholder={'Hi there \n' + '[Interested in "pet-name","pet-info" ]'}></textarea>
								</Stack>
								<Stack className={'info-box'}>
							...
							<Stack className={'left-config'}>
								<Stack className={'prop-desc-config'}>
									<Stack>
										<Stack className={'top'}>
											<Typography className={'title'}>Pet Description</Typography>
											<Typography className={'desc'}>{pet?.petDesc ?? 'No Description!'}</Typography>
										</Stack>
										<Stack className={'bottom'}>
											<Typography className={'title'}>Pet Details</Typography>
											<Stack className={'info-box'}>
												...
												<Stack className={'right'}>
													...
													<Box component={'div'} className={'info'}>
														...
														<Typography className={'data'}>{pet?.petType}</Typography>
													</Box>
													<Box component={'div'} className={'info'}>
									...
									{commentTotal !== 0 && (
										<Stack className={'reviews-config'}>
											...
											<Stack className={'review-list'}>
												{petComments?.map((comment: Comment) => {
						...
						<Stack className="review-addres-config">
							<Stack className={'leave-review-config'}>
								...
								<Box className={'submit-btn'} component={'div'}>
									...
									>
										<Typography className={'title'}>Submit Comments</Typography>
										<svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 17 17" fill="none">
						...
						{destinationPets.length !== 0 && (
							<Stack className={'similar-pets-config'}>
								<Stack className={'title-pagination-box'}>
									<Stack className={'title-box'}>
										<Typography className={'main-title'}>Same posted pets</Typography>
										<Typography className={'sub-title'}>See one more time</Typography>
									</Stack>
								...
								<Stack className={'cards-box'}>
									...
									>
										{destinationPets.map((pet: Pet) => {
											return (
												<SwiperSlide className={'similar-homes-slide'} key={pet.petTitle}>
													<PetBigCard pet={pet} likePetHandler={likePetHandler} key={pet?._id} />
												</SwiperSlide>
											);
										})}
									</Swiper>
